...
server '<server_ip>', user: 'craft', roles: %w{web app db}
...
namespace :deploy do
  ...
  task :copy_config_files do
    on roles(:app) do
      ...
      upload! StringIO.new(File.read("config/nginx.conf")), "/etc/nginx/sites-enabled/#{fetch(:application)}.conf"
    end
  end
